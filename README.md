...
1. Lisää HTML-sivun `<main>` elementtiin toinen `<article>` elementti muokkaamalla tiedostoa js/main-b.js:
   * Uusi elementti `<article>` sisältää samat elementit kuin valmiina oleva elementti `<article>`. 
   * Valitse elementti `<main>` käyttäen [DOM-metodeja](https://www.w3schools.com/js/js_htmldom_elements.asp)
   * Käytä [innerHTML](https://www.w3schools.com/js/js_htmldom_html.asp) ominaisuutta lisätessäsi elementin `<article>` elementtiin `<main>`.
   * Huomaa, että lisäyksessä sinun on käytettävä operaattoria `+=`.
...
1. Vaihda elementin `<script>` src-attribuutin arvoksi main-c.js
1. Lisää uusi elementti `<article>` elementtiin `<main>`:
   * Uusi elementti `<article>` sisältää samat elementit kuin valmiina oleva elementti `<article>`. 
   * Valitse elementti `<main>`  käyttäen [DOM-metodeja](https://www.w3schools.com/js/js_htmldom_elements.asp)
   * Käytä DOM metodeja (createElement, appendChild) lisätessäsi elementin`<article>` elementtiin `<main>`.
...
1. Vaihda elementti `<script>` src-attribuutin arvoksi main-d.js
1. Poista elementti `<article>` elementistä `<main>`.
1. Lisää elementti `<article>` elementtiin `<main>` iteroimalla picArray käyttäen toistorakennetta (main-d.js).
   * Uusi elementti `<article>` sisältää samat elementit kuin valmiina oleva elementti `<article>`.
   * Valitse elementti `<main>` DOM-metodeja käyttäen
   * Käytä DOM-metodeja (createElement, appendChild) tai innerHTML-ominaisuutta lisätäksesi elementin `<article>` elementtiin `<main>`.
      * Lisää otsikko elementtiin `<h2>` ja elementin `<img>` alt-attribuuttiin.
      * Lisää seloste elementtiin `<caption>`.
      * Lisää tiedostopolku elementin `<img>` src-attribuuttiin.
      * Lisää kuvaus elementtiin `<p>`.
